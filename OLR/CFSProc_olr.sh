...
#Path Base
PBase=/home/alrolla/Prono_Semanal/olr
...
PSave=/datos/Prono_Semanal/olr/Data

#Path to climatologies
dClima=/datos/Prono_Semanal/olr/Data/Climatologia/
#Path to R programs
rProg=/home/alrolla/Prono_Semanal/olr/scripts/
...
#Path to anomalies
dAnom=/datos/Prono_Semanal/olr/Data/D${Fecha}/
...
#PROCESO OLR EN EL DIRECTORIO FINAL PARA LA PAGINA.

cd ${dAnom}

for olrfile in ulwtoa.0*
do
    echo $olrfile
    /usr/bin/cdo remapbil,r144x73 $olrfile  ./${olrfile::-3}.R144x73.nc 
done
...
ncks -O --mk_rec_dmn time ${dClima}ulwr_${Fecha:4:4}_S.nc ${dClima}ulwr_${Fecha:4:4}_S.nc

#calculo el promedio semanal de las proximas 4 semanas de la climatologia para ese dia
ncra -O -F -d time,1,7,1    ${dClima}ulwr_${Fecha:4:4}_S.nc ulwr_cli${Fecha:4:4}.week1mean.nc
ncra -O -F -d time,8,14,1   ${dClima}ulwr_${Fecha:4:4}_S.nc ulwr_cli${Fecha:4:4}.week2mean.nc
ncra -O -F -d time,15,21,1  ${dClima}ulwr_${Fecha:4:4}_S.nc ulwr_cli${Fecha:4:4}.week3mean.nc
ncra -O -F -d time,22,28,1  ${dClima}ulwr_${Fecha:4:4}_S.nc ulwr_cli${Fecha:4:4}.week4mean.nc

# Calculo la anomalia con respecto a la climatologia
/usr/bin/Rscript ${rProg}AnomaliasCFS.r "${Fecha}" "ulwr" "OLR" "W/m^2" "ulwtoa" "ulwr_cli" "ULWRF_topofatmosphere"
echo /usr/bin/Rscript ${rProg}AnomaliasCFS.r "${Fecha}" "ulwr" "OLR" "W/m^2" "ulwtoa" "ulwr_cli" "ULWRF_topofatmosphere"
...
${grads}/grads -a 1.680169 -b -l -c "run  ${rProg}plotPronoWeekly.gs ${Fecha} ulwr 1" > /dev/null
${grads}/grads -a 1.680169 -b -l -c "run  ${rProg}plotPronoWeekly.gs ${Fecha} ulwr 2" > /dev/null
${grads}/grads -a 1.680169 -b -l -c "run  ${rProg}plotPronoWeekly.gs ${Fecha} ulwr 3" > /dev/null
${grads}/grads -a 1.680169 -b -l -c "run  ${rProg}plotPronoWeekly.gs ${Fecha} ulwr 4" > /dev/null
...
$gdal/gdal_translate -of PNG -a_srs EPSG:4326 -a_ullr 0. 40.  360. -80.0 anom.ulwr.${Fecha}.week${semana}.jpg anom.ulwr.${Fecha}.week${semana}.png 
...
$gdal/gdalwarp   -t_srs '+proj=merc +pm=180  +a=6378137 +b=6378137 +lat_ts=0.0 +lon_0=0.0 +x_0=0.0 +y_0=0 +k=1.0 +units=m +nadgrids=@null +wktext +no_defs' anom.ulwr.${Fecha}.week${semana}.png anom.ulwr.${Fecha}.week${semana}.tiff  
...
$gdal/gdal_translate -of PNG  anom.ulwr.${Fecha}.week${semana}.tiff anom.ulwr.${Fecha}.week${semana}f.png

#Esto elimina el color blanco de la imagen
${imagemagick}/convert anom.ulwr.${Fecha}.week${semana}f.png -fuzz 10% -transparent white anom.ulwr.${Fecha}.week${semana}f.png


rm anom.ulwr.${Fecha}.week${semana}*.jpg
rm anom.ulwr.${Fecha}.week${semana}.png
rm anom.ulwr.${Fecha}.week${semana}*.xml
rm anom.ulwr.${Fecha}.week${semana}*.tiff
...
ultfile=anom.ulwr.${Fecha}.week${semana}f.png
...
then
	...
	/usr/bin/php -f $rProg/chkFinal.php anom.ulwr.${Fecha}.week${semana}f.png olr
	cp *f.png /var/www/html/CFS/mapas/olr/
     echo ""
#     echo "Continuo con el procesado del PRONO SIS"	 
#     cd /datos/PRONO_SIS
#     ./Run_PRONO_SIS.sh
	 echo "Final exitoso !!!!!!!!!"
	 echo "Final exitoso !!!!!!!!!"
